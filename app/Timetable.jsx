...
const generateFacultyTimetables = (bestTimetable) => {
  ...
  bestTimetable.forEach((classTimeTable) => {
    const { timetable, subject } = classTimeTable;

    timetable.forEach((dailySchedule, dayIndex) => {
      dailySchedule.forEach((slot) => {
        if (slot.subject && slot.subject.faculty) {
          const { name } = slot.subject.faculty;
          const { code, name: subjectName } = slot.subject;
          const { year, div, department } = classTimeTable;
          const timeslot = `${
            getTimeslots(classTimeTable.year)[dailySchedule.indexOf(slot)]
          }`;
          ...
          facultyTimetables[name].push({
            day: dayIndex + 1, // Assuming day index starts from 1
            timeslot,
            subject: `${code} - ${subjectName}`,
            class: `${year} ${department} ${div}`,
